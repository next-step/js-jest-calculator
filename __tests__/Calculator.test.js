...
describe('유효성 검사 테스트', () => {
  ...
  test('입력값이 세자릿수를 넘는 경우 (100 + 1000)', () => {
    expect(Calculator.calculate(100, 1000, OPERATIONS.PLUS)).toThrowError(ERROR_MESSAGE.OVER_MAX_DIGIT);
  });

  test('입력값이 세자릿수를 넘는 경우 (1000 + 100)', () => {
    expect(Calculator.calculate(1000, 100, OPERATIONS.PLUS)).toThrowError(ERROR_MESSAGE.OVER_MAX_DIGIT);
  });
  ...
  test('올바르지 않은 연산자를 입력한 경우 (1 & 1)', () => {
    expect(Calculator.calculate(1, 1, '&')).toThrowError(ERROR_MESSAGE.INVALID_OPERATION);
  });
  ...
  test('여러 에러 중첩시 미입력 => 피연산자 => 연산자 => 자릿수 순으로 출력', () => {
    ...
    expect(Calculator.calculate(1000, 1000, '🐞')).toThrowError(ERROR_MESSAGE.INVALID_OPERATION);
  });
});
