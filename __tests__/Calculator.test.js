...
describe("피연산자 두 개와 연산자 하나의 연산 결과를 반환한다.", () => {
  ...
  it("두 개의 피연산자에 대해 덧셈 결과를 반환한다.", () => {
    expect(calculator._add(pos_operand, neg_operand)).toBe(
      pos_operand + neg_operand
    );
  });

  it("두 개의 피연산자에 대해 뺄셈 결과를 반환한다.", () => {
    expect(calculator._subtract(pos_operand, neg_operand)).toBe(
      pos_operand - neg_operand
    );
  });

  it("두 개의 피연산자에 대해 곱셈 결과를 반환한다.", () => {
    expect(calculator._multiply(pos_operand, neg_operand)).toBe(
      pos_operand * neg_operand
    );
  });

  it("두 개의 피연산자에 대해 나눗셈 결과를 반환한다.", () => {
    expect(calculator._divide(pos_operand, neg_operand)).toBe(
      pos_operand / neg_operand
    );
  });

  it("첫 번째 피연산자는 0이 아니고, 두 번쨰 피연산자가 +0/-0인 경우, 나눗셈 결과 (첫번째 피연산자 부호 * 두 번째 피연산자 부호) * Infinity를 반환한다.", () => {
    expect(calculator._divide(pos_operand, POS_ZERO)).toBe(POS_INF);
    expect(calculator._divide(pos_operand, NEG_ZERO)).toBe(NEG_INF);
    expect(calculator._divide(neg_operand, POS_ZERO)).toBe(NEG_INF);
    expect(calculator._divide(neg_operand, NEG_ZERO)).toBe(POS_INF);
  });

  it("두 피연산자가 모두 0일 때, 나눗셈 결과 NaN을 반환한다.", () => {
    expect(calculator._divide(POS_ZERO, POS_ZERO)).toBeNaN();
    expect(calculator._divide(POS_ZERO, NEG_ZERO)).toBeNaN();
    expect(calculator._divide(NEG_ZERO, POS_ZERO)).toBeNaN();
    expect(calculator._divide(NEG_ZERO, NEG_ZERO)).toBeNaN();
  });
});
...
describe("유효하지 않은 연산 결과가 나오면, 오류 처리한다.", () => {
  it("연산 결과 NaN인 경우, 오류 메시지를 반환한다.", () => {
    expect(() => calculator._validate_ouptut(NaN)).toThrow(
      ERROR_MESSAGE.INVALID_RESULT
    );
  });

  it("연산 결과 -Infinity/Infinity인 경우, 오류 메시지를 반환한다.", () => {
    expect(() => calculator._validate_ouptut(NEG_INF)).toThrow(
      ERROR_MESSAGE.INVALID_RESULT
    );
    expect(() => calculator._validate_ouptut(POS_INF)).toThrow(
      ERROR_MESSAGE.INVALID_RESULT
    );
  });
});

// feature 4 : 유효한 사칙 연산 결과를 조정하여 출력
describe("사칙연산 결과를 조정하여 출력환다.", () => {
